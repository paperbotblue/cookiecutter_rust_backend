...
#[command(
    name = "Diesel CodeGen",
    about = "Generate Rust Diesel service and repo"
)]
struct Args {
    /// Base name (e.g., "type")
    #[arg()]
    ...
    /// Path to the Rust file with domain structs
    #[arg(short, long)]
...
fn append_to_mod_rs(dir: &Path, mod_entry: &str, mod_override: Option<&str>) {
    ...
    let line_to_append = format!("pub mod {};", entry);
...
fn main() {
    ...
    let files_to_generate = vec![
        (
            ...
            format!("{}/domain/services/{}.rs", args.output_dir, type_snake),
        ),
        (
        ...
        (
            ...
            format!(
                ...
                args.output_dir, type_snake
            ),
        ),
        (
            "services.rs.tera",
            format!("{}/services/{}.rs", args.output_dir, type_snake),
        ),
        (
            "infrastructure_model.rs.tera",
            format!(
                "{}/infrastructure/models/{}.rs",
                args.output_dir, type_snake
            ),
        ),
        (
            ...
            format!(
                "{}/api/controllers/{}_handler.rs",
                args.output_dir, type_snake
            ),
        ),
        (
            "api_dto.rs.tera",
            format!("{}/api/dto/{}.rs", args.output_dir, type_snake),
        ),
        (
            "api_routes.rs.tera",
