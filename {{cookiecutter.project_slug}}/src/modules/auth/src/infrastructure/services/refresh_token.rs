...
use crate::infrastructure::repositories::refresh_token::RefreshTokenDieselRepository;
use base::constants;
...
impl RefreshTokenServiceImpl {
    pub fn new(pool: PgPool) -> Self {
        Self {
            repository: Arc::new(RefreshTokenDieselRepository::new(pool)),
        }
    }
}
...
impl RefreshTokenService for RefreshTokenServiceImpl {
    ...
    async fn revoke_refresh_token(&self, id: Uuid) -> Result<(), RefreshTokenError> {
        match self.repository.revoke_family_id(id).await {
            Ok(_) => Ok(()),
            Err(err) => Err(RefreshTokenError::InternalServerError(err)),
        }
    }
    ...
    fn verify_jwt(&self, token: &str) -> Result<JwtClaims, RefreshTokenError> {
        ...
        .map_err(|err| match err.kind() {
            jsonwebtoken::errors::ErrorKind::ExpiredSignature => RefreshTokenError::TokenExpired,
