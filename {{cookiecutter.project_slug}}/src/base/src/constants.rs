...
lazy_static! {
    ...
    pub static ref OTP_EXP_MINUTES: i64 = set_otp_exp_minutes();
...
fn set_password_hash_secret() -> String {
    dotenv::dotenv().ok();
    env::var("PASSWORD_HASH_SECRET").expect("Password hash Secret not found in env")
}
...
fn set_otp_exp_minutes() -> i64 {
    dotenv::dotenv().ok();
    env::var("OTP_EXP_MINUTES")
        .unwrap()
        .parse::<i64>()
        .expect("OTP exp not set")
}
